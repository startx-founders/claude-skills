...
`/startx-boa-prep` in Claude Code, from your working or notes directory. Hand it your deck, wiki, or notes; it reads those first and asks only the gaps, building a resumable pre-read. It writes `boa-prep-YYYY-MM-DD.md` plus a cover-note outline (points you write your note from), labels every output SEND AHEAD or KEEP INTERNAL, and runs a send-safety pass before anything reaches your board.
...
v1.2 (2026-06-24). Companion to `startx-office-hours-prep` and `startx-boa-debrief`. History in the SKILL.md CHANGELOG.
